...
- **A rejected input names its field only in `extensions.validationErrors`.** The `message` for one is `Argument Validation Error` — neither the field nor the rule — while the array beside it carries `property` and `constraints`. `APIError.Error()` renders those beneath the message, so the whole diagnosis is in the one place every diagnostic already goes through `err.Error()`. Terraform's plan cannot compensate: it accepts any string, and only the API objects, at apply time.
